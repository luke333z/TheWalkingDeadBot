...
module.exports = {
    ...
    execute(message, args){
        ...
        if(args[0] === "random"){
            ...
            if(args[0] === "invite"){
                const serverinfo = new Discord.MessageEmbed().setColor("RANDOM").addField("**Usage:** `-invite`", "Shows the bot's invite.").setFooter(`${process.env.DEVELOPER} •`);
                message.channel.send(serverinfo);
            }
            if(args[0] === "loot"){
            ...
            .addField("**UTIL**", "`avatar`, `userinfo`, `serverinfo`, `poll`, `botinfo`, `commands`, `invite`, `suggest`")
